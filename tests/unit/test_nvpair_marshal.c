...
static bool
nvl_exists_type(nvlist_t *nv, const char *name, data_type_t type)
{
	nvpair_t *pair;
	int err = nvlist_lookup_nvpair(nv, name, &pair);
	if (err != 0)
		return (false);
	return (nvpair_type(pair) == type);
}
...
test_nvm_flag_true(const MunitParameter params[], void *data)
{
	...
	nvlist_t *out = fnvlist_alloc();
	unit_ok(NVM_MARSHAL(out, nvm_flag_t, &a));
	unit_true(nvl_exists_type(out, "flag", DATA_TYPE_BOOLEAN));
...
test_nvm_opt_scalar_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "val", DATA_TYPE_UINT64));
...
test_nvm_opt_array_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "arr", DATA_TYPE_UINT64_ARRAY));
...
static MunitResult
test_nvm_opt_array_empty(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	unit_ok(nvlist_add_uint64_array(in, "arr", NULL, 0));
...
test_nvm_opt_struct_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "nv", DATA_TYPE_NVLIST));
...
test_nvm_opt_struct_array_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "arr", DATA_TYPE_NVLIST_ARRAY));
...
static MunitResult
test_nvm_opt_struct_array_empty(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	unit_ok(nvlist_add_nvlist_array(in, "arr", NULL, 0));
...
test_nvm_opt_map_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "map", DATA_TYPE_NVLIST));
...
test_nvm_opt_set_true(const MunitParameter params[], void *data)
{
	...
	unit_true(nvl_exists_type(out, "set", DATA_TYPE_NVLIST));
...
NVM_SCHEMA(nvm_opt_zero_scalar_t,
	("b",       NVM_SCALAR(BOOLEAN),                        b,       NVM_OPTIONAL),
	("byte",    NVM_SCALAR(BYTE),                        byte,       NVM_OPTIONAL),
	("i8",     NVM_SCALAR(INT8),                         i8,     NVM_OPTIONAL),
	("u8",     NVM_SCALAR(UINT8),                         u8,     NVM_OPTIONAL),
	("i16",     NVM_SCALAR(INT16),                         i16,     NVM_OPTIONAL),
	("u16",     NVM_SCALAR(UINT16),                         u16,     NVM_OPTIONAL),
	("i32",     NVM_SCALAR(INT32),                         i32,     NVM_OPTIONAL),
	("u32",     NVM_SCALAR(UINT32),                         u32,     NVM_OPTIONAL),
	("i64",     NVM_SCALAR(INT64),                         i64,     NVM_OPTIONAL),
	("u64",     NVM_SCALAR(UINT64),                         u64,     NVM_OPTIONAL),
	("ts",     NVM_SCALAR(HRTIME),                         ts,     NVM_OPTIONAL),
	("d",     NVM_SCALAR(DOUBLE),                         d,     NVM_OPTIONAL),
	("str",     NVM_SCALAR(STRING),                         str,     NVM_OPTIONAL),
	("nv",      NVM_SCALAR(NVLIST),                         nv,      NVM_OPTIONAL),
);

static MunitResult
test_nvm_opt_zero_scalar(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_zero_scalar_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_zero_scalar_t, &a));
	...
	unit_false(a.has_b);
	unit_false(a.has_byte);
	unit_false(a.has_i8);
	unit_false(a.has_u8);
	unit_false(a.has_i16);
	unit_false(a.has_u16);
	unit_false(a.has_i32);
	unit_false(a.has_u32);
	unit_false(a.has_i64);
	unit_false(a.has_u64);
	unit_false(a.has_ts);
	unit_false(a.has_d);
...
NVM_SCHEMA(nvm_opt_zero_array_t,
	("b_arr",     NVM_ARRAY(BOOLEAN),                        b_arr,     NVM_OPTIONAL),
	("byte_arr",     NVM_ARRAY(BYTE),                        byte_arr,     NVM_OPTIONAL),
	("i8_arr",     NVM_ARRAY(INT8),                        i8_arr,     NVM_OPTIONAL),
	("u8_arr",     NVM_ARRAY(UINT8),                        u8_arr,     NVM_OPTIONAL),
	("i16_arr",     NVM_ARRAY(INT16),                        i16_arr,     NVM_OPTIONAL),
	("u16_arr",     NVM_ARRAY(UINT16),                        u16_arr,     NVM_OPTIONAL),
	("i32_arr",     NVM_ARRAY(INT32),                        i32_arr,     NVM_OPTIONAL),
	("u32_arr",     NVM_ARRAY(UINT32),                        u32_arr,     NVM_OPTIONAL),
	("i64_arr",     NVM_ARRAY(INT64),                        i64_arr,     NVM_OPTIONAL),
	("u64_arr",     NVM_ARRAY(UINT64),                        u64_arr,     NVM_OPTIONAL),
	("str_arr",     NVM_ARRAY(STRING),                        str_arr,     NVM_OPTIONAL),
	("nv_arr",     NVM_ARRAY(NVLIST),                        nv_arr,     NVM_OPTIONAL),
);

static MunitResult
test_nvm_opt_zero_array(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_zero_array_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_zero_array_t, &a));

	unit_null(a.b_arr);
	unit_null(a.byte_arr);
	unit_null(a.i8_arr);
	unit_null(a.u8_arr);
	unit_null(a.i16_arr);
	unit_null(a.u16_arr);
	...
	unit_false(a.has_b_arr);
	unit_false(a.has_byte_arr);
	unit_false(a.has_i8_arr);
	unit_false(a.has_u8_arr);
	unit_false(a.has_i16_arr);
	unit_false(a.has_u16_arr);
	unit_false(a.has_i32_arr);
	unit_false(a.has_u32_arr);
	unit_false(a.has_i64_arr);
	unit_false(a.has_u64_arr);
	unit_false(a.has_str_arr);
	unit_false(a.has_nv_arr);

	fnvlist_free(in);
	return (MUNIT_OK);
}

NVM_SCHEMA(nvm_opt_zero_array_n_t,
	("b_arr",     NVM_ARRAY_N(BOOLEAN, 5),                        b_arr,     NVM_OPTIONAL),
	("byte_arr",     NVM_ARRAY_N(BYTE, 5),                        byte_arr,     NVM_OPTIONAL),
	("i8_arr",     NVM_ARRAY_N(INT8, 5),                        i8_arr,     NVM_OPTIONAL),
	("u8_arr",     NVM_ARRAY_N(UINT8, 5),                        u8_arr,     NVM_OPTIONAL),
	("i16_arr",     NVM_ARRAY_N(INT16, 5),                        i16_arr,     NVM_OPTIONAL),
	("u16_arr",     NVM_ARRAY_N(UINT16, 5),                        u16_arr,     NVM_OPTIONAL),
	("i32_arr",     NVM_ARRAY_N(INT32, 5),                        i32_arr,     NVM_OPTIONAL),
	("u32_arr",     NVM_ARRAY_N(UINT32, 5),                        u32_arr,     NVM_OPTIONAL),
	("i64_arr",     NVM_ARRAY_N(INT64, 5),                        i64_arr,     NVM_OPTIONAL),
	("u64_arr",     NVM_ARRAY_N(UINT64, 5),                        u64_arr,     NVM_OPTIONAL),
	("str_arr",     NVM_ARRAY_N(STRING, 5),                        str_arr,     NVM_OPTIONAL),
	("nv_arr",     NVM_ARRAY_N(NVLIST, 5),                        nv_arr,     NVM_OPTIONAL),
);

static MunitResult
test_nvm_opt_zero_array_n(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_zero_array_n_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_zero_array_n_t, &a));

	for (uint_t i = 0; i < ARRAY_SIZE(a.b_arr); i++) {
		unit_zero(a.b_arr[i]);
		unit_zero(a.byte_arr[i]);
		unit_zero(a.i8_arr[i]);
		unit_zero(a.u8_arr[i]);
		unit_zero(a.i16_arr[i]);
		unit_zero(a.u16_arr[i]);
		unit_zero(a.i32_arr[i]);
		unit_zero(a.u32_arr[i]);
		unit_zero(a.i64_arr[i]);
		unit_zero(a.u64_arr[i]);
		unit_null(a.str_arr[i]);
		unit_null(a.nv_arr[i]);
	}
...
NVM_SCHEMA(nvm_opt_zero_struct_inner_t,
	("val",  NVM_SCALAR(UINT64),   val,    NVM_OPTIONAL),
);

NVM_SCHEMA(nvm_opt_zero_struct_t,
	("st",      NVM_STRUCT(nvm_opt_zero_struct_inner_t),        st,      NVM_OPTIONAL),
	("st_arr",  NVM_STRUCT_ARRAY(nvm_opt_zero_struct_inner_t),  st_arr,  NVM_OPTIONAL),
);

static MunitResult
test_nvm_opt_zero_struct(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_zero_struct_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_zero_struct_t, &a));

	unit_zero(a.st.val);
	unit_false(a.st.has_val);
	unit_false(a.has_st);

	unit_null(a.st_arr);
	unit_false(a.has_st_arr);
	unit_zero(a.nelem_st_arr);
...
NVM_SCHEMA(nvm_opt_default_scalar_t,
	("b",       NVM_SCALAR(BOOLEAN),  b,       NVM_DEFAULT(B_TRUE)),
	("byte",    NVM_SCALAR(BYTE),     byte,    NVM_DEFAULT(0xa5)),
	("i8",      NVM_SCALAR(INT8),     i8,      NVM_DEFAULT(-1)),
	("u8",      NVM_SCALAR(UINT8),    u8,      NVM_DEFAULT(1)),
	("i16",     NVM_SCALAR(INT16),    i16,     NVM_DEFAULT(-300)),
	("u16",     NVM_SCALAR(UINT16),   u16,     NVM_DEFAULT(300)),
	("i32",     NVM_SCALAR(INT32),    i32,     NVM_DEFAULT(-12345678)),
	("u32",     NVM_SCALAR(UINT32),   u32,     NVM_DEFAULT(12345678)),
	("i64",     NVM_SCALAR(INT64),    i64,     NVM_DEFAULT(-12345678901234)),
	("u64",     NVM_SCALAR(UINT64),   u64,     NVM_DEFAULT(12345678901234)),
	("ts",      NVM_SCALAR(HRTIME),   ts,      NVM_DEFAULT(1000000000)),
	("str",     NVM_SCALAR(STRING),   str,     NVM_DEFAULT("hello")),
);

static MunitResult
test_nvm_opt_default_scalar(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_default_scalar_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_default_scalar_t, &a));
	...
	unit_eq(a.i16, -300);
	unit_eq(a.u16, 300);
	unit_eq(a.i32, -12345678);
	unit_eq(a.u32, 12345678);
	unit_eq(a.i64, -12345678901234);
	unit_eq(a.u64, 12345678901234);
	unit_eq(a.ts, 1000000000);
	unit_str_eq(a.str, "hello");
...
NVM_SCHEMA(nvm_opt_default_struct_inner_t,
	("val",  NVM_SCALAR(UINT64),   val,    NVM_DEFAULT(23)),
);

NVM_SCHEMA(nvm_opt_default_struct_t,
	("st",      NVM_STRUCT(nvm_opt_default_struct_inner_t),        st,      NVM_OPTIONAL),
);

static MunitResult
test_nvm_opt_default_struct(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvlist_t *in = fnvlist_alloc();

	nvm_opt_default_struct_t a;
	memset(&a, 0x5a, sizeof (a));
	unit_ok(NVM_UNMARSHAL(in, nvm_opt_default_struct_t, &a));
...
test_nvm_spill(const MunitParameter params[], void *data)
{
	...
	nvlist_t *out = fnvlist_alloc();
	unit_ok(NVM_MARSHAL(out, nvm_spill_t, &a));

	unit_eq(fnvlist_num_pairs(out), 3);

	unit_eq(fnvlist_lookup_uint64(out, "foo"), 1);
	unit_eq(fnvlist_lookup_uint64(out, "bar"), 2);
	unit_eq(fnvlist_lookup_uint64(out, "baz"), 3);
...
static MunitResult
test_nvm_spill_conflict(const MunitParameter params[], void *data)
{
	(void) params; (void) data;

	nvm_pair_uint64_t spill[] = {
		{ "foo", 2 },
	};
	...
	a.foo = 1;
	a.spill = spill;
	a.nelem_spill = ARRAY_SIZE(spill);

	nvlist_t *out = fnvlist_alloc();
	unit_err(NVM_MARSHAL(out, nvm_spill_t, &a), EEXIST);
...
test_nvm_complex(const MunitParameter params[], void *data)
{
	...
	if (a.flag)
		unit_true(nvl_exists_type(top, "flag", DATA_TYPE_BOOLEAN));
	...
	for (uint_t i = 0; i < ARRAY_SIZE(elems); i++) {
		...
		if (nvlist_exists(outelem, "flag")) {
			unit_true(nvl_exists_type(outelem, "flag", DATA_TYPE_BOOLEAN));
			unit_true(nvl_exists_type(inelem, "flag", DATA_TYPE_BOOLEAN));
		} else
			unit_false(nvlist_exists(inelem, "flag"));
...
static const MunitTest nvpair_marshal_tests[] = {
	...
	UNIT_TEST("nvm_opt_array_empty",	test_nvm_opt_array_empty),
	...
	UNIT_TEST("nvm_opt_struct_array_empty",	test_nvm_opt_struct_array_empty),
	...
	UNIT_TEST("nvm_opt_zero_scalar",	test_nvm_opt_zero_scalar),
	UNIT_TEST("nvm_opt_zero_array",	test_nvm_opt_zero_array),
	UNIT_TEST("nvm_opt_zero_array_n",	test_nvm_opt_zero_array_n),
	UNIT_TEST("nvm_opt_zero_struct",	test_nvm_opt_zero_struct),

	UNIT_TEST("nvm_opt_default_scalar",	test_nvm_opt_default_scalar),
	UNIT_TEST("nvm_opt_default_struct",	test_nvm_opt_default_struct),
	...
	UNIT_TEST("nvm_spill_conflict",		test_nvm_spill_conflict),
