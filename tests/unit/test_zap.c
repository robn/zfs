...
test_norm_nfkc(const MunitParameter params[], void *data)
{
	...
	/* "ﬁle": fi ligature (U+FB01 = \xef\xac\x81) + "le" vs plain "file" */
...
test_norm_nfkd(const MunitParameter params[], void *data)
{
	...
	uint64_t val = 42;
	unit_ok(zap_add_by_dnode(dn, plain_key, sizeof (uint64_t), 1, &val, tx));
...
test_norm_conflict(const MunitParameter params[], void *data)
{
	...
	/* Insert two distinct keys that normalise to the same form. */
	uint64_t v1 = 1, v2 = 2;
...
/*
 * zap_remove_norm_by_dnode: remove by normalized name.
 *
 * MT_NORMALIZE: "hello" removes the stored key "Hello".
 * MT_EXACT: "hello" does not remove "Hello"; "Hello" does.
 */
static MunitResult
