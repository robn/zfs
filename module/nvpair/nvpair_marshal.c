...
static int
_nvm_marshal_pair(nvlist_t *nv, nvm_kind_t k, const char *name,
...
static int
_nvm_marshal_map(nvlist_t *nv, nvm_kind_t k, const char *name,
    nvm_kind_u *u, uint_t nelem)
{
	int err = 0;
	...
	for (uint_t elem = 0; err == 0 && elem < nelem; elem++) {
		if (k == NVMK_FLAG)
			err = nvlist_add_boolean(map, u->str_arr[elem]);
		else {
			nvm_kind_u *eu = (nvm_kind_u *)&u->map_arr[elem].value;
			err = _nvm_marshal_pair(map, k,
			    u->map_arr[elem].name, eu, elem);
		}
	}
...
static int
_nvm_marshal_one(nvlist_t *nv, const nvm_field_t *f,
    nvm_kind_u *u, uint_t nelem)
{
	...
	if (f->nvmf_flags & NVMF_MAP)
		return (_nvm_marshal_map(nv, k, name, u, nelem));

	if (k >= NVMK_BOOLEAN && k <= NVMK_NVLIST_ARRAY)
		return (_nvm_marshal_pair(nv, k, name, u, nelem));
...
nvm_marshal(nvlist_t *nv, const nvm_desc_t *desc, void *a)
{
	...
	for (size_t i = 0; err == 0 && i < desc->nvmd_nfields; i++) {
		...
		err = _nvm_marshal_one(nv, f, u, nelem);
	}
...
static int
_nvm_unmarshal_pair(nvpair_t *pair, nvm_kind_t k, nvm_kind_u *u, uint_t *nelemp)
{
	int err = 0;
...
static int
_nvm_unmarshal_map(nvlist_t *nv, nvm_kind_t k, nvm_kind_u *u,
	...
	*nelemp = fnvlist_num_pairs(nv);
	...
	for (nvpair_t *ep = nvlist_next_nvpair(nv, NULL);
		...
		if (k == NVMK_FLAG) {
			...
			err = _nvm_unmarshal_pair(ep, k, eu, NULL);
...
static int
_nvm_unmarshal_one(nvlist_t *nv, const nvm_field_t *f,
    nvm_kind_u *u, uint_t *nelemp)
{
	...
	if (f->nvmf_flags & NVMF_MAP) {
		...
		if (err == 0)
			err = _nvm_unmarshal_map(map, f->nvmf_kind, u, nelemp,
	 ...
	 */
	if (k >= NVMK_BOOLEAN && k <= NVMK_NVLIST_ARRAY) {
		err = _nvm_unmarshal_pair(pair, f->nvmf_kind, u, nelemp);
		if (err == EINVAL)
			err = ENOENT;
		return (err);
	}
...
nvm_unmarshal(nvlist_t *nv, const nvm_desc_t *desc, void *a)
{
	...
	for (size_t i = 0; err == 0 && i < desc->nvmd_nfields; i++) {
		...
		ferr = _nvm_unmarshal_one(nv, f, u, nelemp);
	...
	if (err == 0 && spill != NULL) {
		...
		err = _nvm_unmarshal_map(nv, spill->nvmf_kind, u, nelemp,
	...
	for (size_t i = 0; i < desc->nvmd_nfields; i++) {
		...
		if (f->nvmf_flags & NVMF_OPTIONAL)
			*(boolean_t *)(base + f->nvmf_has_offset) = B_FALSE;
	}
