...
{
	zfs_mountset_t *mset = libzfs_mountset_enter(zfs_hdl);

	zfs_mount_t *mnt;
	if (zfs_mountset_find_dataset(mset, special, &mnt) != 0)
		return (B_FALSE);

	if (where != NULL)
		*where = zfs_strdup(zfs_hdl, zfs_mount_get_mountpoint(mnt));
...
unmount_one(libzfs_handle_t *hdl, const char *mountpoint, int flags)
{
	...
	/*
	 * XXX temp hack; both implementations of do_unmount() ignore their
	 *     first arg, so we don't have to pass a dataset handle down
	 *     anymore, which is good because we don't have one anymore.
	 *     this function doesn't need one either, but we're passing hdl
	 *     temporarily to keep the error output alive. this was done
	 *     for zpool_disable_datasets(), which doesn't need a handle in
	 *     the disable_unmount_cb() stage
	 *       -- robn, 2026-03-14
	 */
	error = do_unmount(NULL, mountpoint, flags);
	if (error != 0) {
		int libzfs_err;
    ...
    const enum sa_protocol *proto)
{
	...
	if (mountpoint == NULL) {
		if (zfs_get_type(zhp) == ZFS_TYPE_FILESYSTEM) {
			mset = libzfs_mountset_enter(hdl);

			zfs_mount_t *mnt;
			int err = zfs_mountset_find_dataset(mset,
			    zhp->zfs_name, &mnt);
			if (err == 0)
				mntpt = zfs_mount_get_mountpoint(mnt);
			else if (err != ENOENT) {
				zfs_mountset_exit(mset);
				return (-1);
			}
